...
for index, input in enumerate(inputs):
    ...
    if name != '':
        named[name] = inputs[index][1].replace(' ', '')
...
def calculate(start, path):
    ...
    if 'RSHIFT' in path[start]:
        name = path[start].split('RSHIFT')[0]
        value = path[start].split('RSHIFT')[1]
        path[start] = str(int(calculate(name, path)) >> int(value))

    if 'LSHIFT' in path[start]:
        name = path[start].split('LSHIFT')[0]
        value = path[start].split('LSHIFT')[1]
        path[start] = str(int(calculate(name, path)) << int(value))

    if 'AND' in path[start]:
        name = path[start].split('AND')[0]
        value = path[start].split('AND')[1]
        path[start] = str(int(calculate(name, path)) & int(calculate(value, path)))

    if 'OR' in path[start]:
        name = path[start].split('OR')[0]
        value = path[start].split('OR')[1]
        path[start] = str(int(calculate(name, path)) | int(calculate(value, path)))

    if 'NOT' in path[start]:
        name = path[start].split('NOT')[1]
